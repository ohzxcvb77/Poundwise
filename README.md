...
영국에서 생활하는 학생과 가족을 위한 GBP/KRW 이중 통화 가계부입니다. 웹에서 바로 사용할 수 있고, HTTPS로 배포하면 Android/iPhone 홈 화면에 설치되는 PWA로 동작합니다. 기본은 로컬 저장이며, Supabase를 연결하면 가족 공유와 기기 간 동기화를 선택적으로 사용할 수 있습니다.
...
- 이메일 계정 로그인, 가족 가계부 생성, 8자리 초대 코드 참여
...
표시된 로컬 주소를 브라우저에서 열면 됩니다. 단순히 `index.html`을 더블클릭해도 로컬 가계부 기능은 동작하지만, 앱 설치·오프라인 캐시·일부 클라우드 기능에는 HTTPS 또는 `localhost` 주소가 필요합니다.
...
클라우드 기능은 Supabase 프로젝트가 준비된 뒤 활성화됩니다. 프로젝트 URL과 브라우저용 공개 키가 없는 상태에서는 기존처럼 로컬 전용 가계부로 동작합니다.
...
5. 프로젝트의 Data API/Connect 화면에서 Project URL과 `Publishable key` 또는 레거시 `anon key`를 확인합니다.
...
1. `예산 및 공유 설정 → 가족 공유 · 기기 동기화`를 엽니다.
2. `연결 설정 열기`를 누릅니다.
3. Supabase Project URL과 Publishable/anon key를 입력하고 저장합니다.

연결 정보는 현재 브라우저에 저장됩니다.

#### 배포 파일에 미리 설정

`cloud-config.js`를 수정하면 가족이 같은 배포 주소에서 별도 설정 없이 로그인할 수 있습니다.
...
1. 이메일 계정을 만들고 로그인합니다.
2. `새 가족 가계부 만들기`에서 이름과 표시 이름을 입력합니다.
3. 생성된 8자리 초대 코드를 가족에게 전달합니다.

다른 가족:

1. 같은 앱 주소에서 자신의 이메일 계정으로 로그인합니다.
2. `초대 코드로 참여`에 코드와 표시 이름을 입력합니다.
3. 참여가 완료되면 양쪽 기기의 거래와 예산이 자동으로 합쳐집니다.

동기화는 거래 추가·수정·삭제 직후, 앱을 다시 열 때, 인터넷 연결이 복구될 때, 그리고 20초 간격으로 실행됩니다. 동시에 수정된 항목은 `updated_at`이 더 최근인 변경을 유지합니다. 삭제 내역도 tombstone으로 동기화되어 다른 기기에서 다시 나타나지 않습니다.
...
- 앱 데이터: `localStorage`의 `poundwise_state_v1`
- 마지막 성공 환율: `localStorage`의 `poundwise_rate_cache_v1`
- 클라우드 연결 정보: `localStorage`의 `poundwise_cloud_config_v1`
- 로컬 모드에서는 거래 데이터를 외부로 전송하지 않습니다.
- 가족 공유를 만들거나 참여하면 현재 거래·예산이 사용자가 설정한 Supabase 프로젝트로 동기화됩니다.
